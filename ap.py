...
<style>
    ...
    div[data-testid="stRadio"] > div[role="radiogroup"] {
        ...
        gap: 35px; margin-top: 10px; margin-bottom: 25px;
        flex-wrap: wrap; background-color: transparent !important; 
    }
    div[data-testid="stRadio"] > div[role="radiogroup"] > label {
        cursor: pointer; background: transparent !important;
        border: none !important; box-shadow: none !important; padding: 5px !important;
    }
...
# ================= 8. BẢNG DỮ LIỆU, ĐƯỜNG KẺ & TẢI XUỐNG =================
with table_container:
    ...
    # Dải màu phân cách điểm nhấn cho tiêu đề bảng
    st.markdown("""<div style="height: 5px; background: linear-gradient(to right, #C62828, #198754); border-radius: 5px; margin-bottom: 10px;"></div>""", unsafe_allow_html=True)

    # Nút tải xuống dữ liệu CSV đã lọc
    csv = df_display.to_csv(index=False).encode('utf-8-sig')
    st.download_button(
        label="📥 Tải xuống dữ liệu bảng đã lọc (CSV)",
        data=csv,
    ...
    def color_rows(row):
        ...
        if status == 'Chưa bắt đầu': return ['background-color: #f5f5f5; color: #000000;'] * len(row)
        return [''] * len(row)

    styled_df = df_display.style.apply(color_rows, axis=1)
    
    # Sử dụng st.data_editor để tiêu đề bảng hiển thị sắc nét, chuyên nghiệp và đầy đủ
